...
datos.mod1 <- list(
  O = aragon.spdf$O,          # Observaciones de muertes
  E = aragon.spdf$E,          # Observaciones de exposiciones
  adj = vecinos$adj,          # Vecindad entre áreas
  num = vecinos$num,          # Número de vecinos por área
  w = vecinos$weights,        # Pesos de las conexiones vecinales
  nObs = length(aragon.spdf)  # Número de observaciones (áreas)
)
...
res.mod1 <- bugs(
  data = datos.mod1,      # Datos del modelo
  model = mod1,           # Función que define el modelo
  param = params.mod1,    # Parámetros que queremos obtener
  inits = inits.mod1      # Valores iniciales del modelo
)
...
### --- 2. Verificando que los datos del shapefile y el data.frame coincidan --- ####
...
### ------- 2.1.1. ISQ --- ####
# Creación de una variable para la tasa de suicidio (ISQ)
aragon_shape$isq_raw <- data$O / data$E  # Tasa de suicidio (observado/esperado)

# Definir los rangos para la categorización de ISQ
isq_raw.cutoff <- c(0.6, 0.9, 1.3, 1.7)
...
# Crear la relación de vecinos espaciales usando el shapefile
...
### --- 4.4. Distribución posterior de la mortalidad por suicidio --- ####
# Almacenar las distribuciones posteriores de la tasa de suicidio
aragon_shape$isq_mean <- mod.isq$summary.fitted.values$mean    # Media
aragon_shape$isq_sd <- mod.isq$summary.fitted.values$sd        # Desviación estándar
aragon_shape$isq_median <- mod.isq$summary.fitted.values$`0.5quant`  # Mediana
aragon_shape$isq_q025 <- mod.isq$summary.fitted.values$`0.025quant` # Cuantil 2.5%
aragon_shape$isq_q975 <- mod.isq$summary.fitted.values$`0.975quant` # Cuantil 97.5%
aragon_shape$isq_p1 <- 1 - mod.isq$summary.fitted.values$`1cdf`  # Probabilidad de ser mayor que 1
...
# Resultados: Mapas de RME y RME \> 1
...
#| echo: false
#| fig.align: "center"
#| fig.caption: "Mapeo de modelos realizados por INLA y WINGBUGS"
...
final_plot <- (rme_plot | inla_mean_plot ) /
              (p_rme_plot| inla_p_plot)
...
# Correlación entre Resultados de WingBUGS e INLA
...
#| warning: false
#| fig.align: "center"
#| fig.caption: "Disperciones entre modelos"
...
- WinBUGS: Utiliza simulaciones Monte Carlo, lo que permite explorar con mayor profundidad la incertidumbre en las estimaciones, pudiendo generar ligeras variaciones en las colas de la distribución. 
...
#| fig.width: 8
#| fig.align: "center"
#| fig.caption: "Gráfico de densidad de la media"


Datos_medias <- data.frame("winbugs" = res.mod1$mean$R, "INLA" = mod.isq$summary.fitted.values$mean)
